...
#define gf_dbg(fmt, args...) do { \
		pr_warn("goodix-drv:" fmt, ##args);\
	} while (0)
#define FUNC_ENTRY()  pr_warn("goodix-drv:%s, entry\n", __func__)
#define FUNC_EXIT()  pr_warn("goodix-drv:%s, exit\n", __func__)
#else
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_SENDKEY:
		...
		pr_info("GF_IOC_SENDKEY %s: gf_key.key = %d, gf_key.value = %d.\n",__func__,
	...
	case GF_IOC_RELEASEVERSION: {
		...
		if (!ret) {
			...
			gf_dbg("GF_IOC_RELEASEVERSION -Failed to copy data from user space:%d\n", ret);
		}
	}
	break;
	default:
...
static int __init gf_init(void)
{
	...
	pr_info(" platform_driver_register status = 0x%x\n", status);
#elif defined(USE_SPI_BUS)
	status = spi_register_driver(&gf_driver);
	pr_info(" spi_register_driver status = 0x%x\n", status);
#endif
	if (status < 0) {
